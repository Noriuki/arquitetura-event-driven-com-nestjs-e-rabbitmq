...
@Injectable()
export class HandlePaymentCompletedUseCase {
  constructor(
  ...
  async execute(data: unknown): Promise<void> {
    ...
    if (!orderId) {
      throw new Error('payment.completed payload missing orderId');
    }
    await this.transition.execute(orderId, OrderStatus.CONFIRMED);
  }
}
